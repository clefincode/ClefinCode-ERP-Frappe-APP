  ...
  async create_app() {
        ...
        if(attr=='data-room'){
            ...
            if(chat_list_obj){
                ...
                if(room_type == 'Contributor'){
                    frappe.call({
                        method:
                          "clefincode_erp.api.api_1_0_1.api.mark_messsages_as_read",
                        args: {
                          user: me.user_email ,
                          channel: null,
    ...
    const navbar_icon_html = `
        ...
        dropdown-mobile chat-navbar-icon' title="Show Chats" >
          <img title="Show Chats" src="/assets/clefincode_erp/icons/clefincode-chat.svg" width="25px" height="25px">
...
async function get_settings(token) {
  const res = await frappe.call({
    type: "GET",
    method: "clefincode_erp.api.api_1_0_1.api.get_settings",
    args: {
      token: token,
    },
  });
...
async function calculate_unread_messages(user) {
  const res = await frappe.call({
    type: "GET",
    method: "clefincode_erp.api.api_1_0_1.api.calculate_unread_messages",
    args: {
      user: user,
    },
  });
