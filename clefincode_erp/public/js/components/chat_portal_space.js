...
async function create_guest_profile_and_channel(
  ...
  const res = await frappe.call({
    method:
      "clefincode_erp.api.api_1_0_1.chat_portal.create_guest_profile_and_channel",
    args: {
      content: content,
      sender: sender,
...
async function send_message(params) {
  const { content, room, sender, sender_email, send_date, respondent_user } =
    params;
  const res = await frappe.call({
    method: "clefincode_erp.api.api_1_0_1.chat_portal.send",
    args: {
      content: content,
      room: room,
      sender: sender,
...
async function get_messages(room) {
  const res = await frappe.call({
    method: "clefincode_erp.api.api_1_0_1.chat_portal.get_messages",
    args: {
      room: room,
    },
  });
...
async function get_respondent_user(room) {
  const res = await frappe.call({
    method: "clefincode_erp.api.api_1_0_1.chat_portal.get_respondent_user",
    args: {
      room: room,
    },
  });
